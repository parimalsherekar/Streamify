...
io.on("connection",(socket)=>{
    ...
    socket.on("join-room",({roomId,userName,userId})=>{
        ...
        if(!rooms[roomId]){
            ...
            socket.join(roomId);
            io.to(socket.id).emit("user-list",{userList:rooms[roomId]});
            rooms[roomId].push({socketId:socket.id,userName,userId});
            io.to(socket.id).emit("server-msg",`${roomId} has been joined succesfully`);
            io.to(roomId).emit("new-joinee",{socketId:socket.id});
        }
    });
    ...
    socket.on("leave-room",({roomId})=>{
        ...
        if(rooms[roomId].length <= 0){
            ...
            socket.to(roomId).emit("server-msg",`${userName} has left the room`);
        }
    });

    socket.on("offer",({offer,from,to})=>{
        io.to(to).emit("offer",{offer,from,to});
    });
    ...
    socket.on("ice-candidate",({candidate,from,to})=>{
        io.to(to).emit("ice-candidate",({candidate,from,to}));
    })
})
